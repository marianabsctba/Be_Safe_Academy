...
def fetch_feed(source, url):
    ...
    for node in nodes[:12]:
        ...
        url = link(node)
        if title and url:
            rows.append({"source": source, "original_title": title[:300], "original_summary": text(node, {"description", "summary", "content"})[:700], "url": url, "published": iso_date(text(node, {"pubdate", "published", "updated", "date"})), "image": image(node)})
    return rows
...
def translate_batch(items, token):
    ...
    body = json.dumps({"model": os.getenv("NEWS_MODEL", "openai/gpt-4.1-mini"), "temperature": 0.1, "response_format": {"type": "json_object"}, "messages": [{"role": "system", "content": prompt}, {"role": "user", "content": json.dumps([{"index": i, "source": x["source"], "title": x["original_title"], "description": x["original_summary"]} for i, x in enumerate(items)], ensure_ascii=False)}]}).encode()
    ...
    return json.loads(result["choices"][0]["message"]["content"])["items"]
